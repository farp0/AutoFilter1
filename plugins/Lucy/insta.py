...
async def link_handler(Mbot, message):
    ...
    try:
        ...
        # Forward to the dump group if file is available
        ...
        try:
            ...
            if "/reel/" in url:
                ...
                # Send the video or download file
                try:
                    if ddinsta:
            ...
            # Handle Instagram photo posts (new logic added here)
            elif "/p/" in url:
                meta_tag = requests.post("https://saveig.app/api/ajaxSearch", data={"q": link, "t": "media", "lang": "en"}, headers=headers)
                if meta_tag.ok:
                    res = meta_tag.json()
                ...
                # Iterate over the photo links
                for i in range(len(meta) - 1):
                    com = await message.reply_text(meta[i])
                    await asyncio.sleep(1)
                    try:
                        dump_file = await message.reply_photo(com.text, caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @Lucy_Filter_bot")
                        await com.delete()
                    except:
                        pass
            ...
            await message.reply(f"400: Sorry, Unable To Find It. Please try another or report it to @VeldXd or the support chat: https://t.me/+DnmZbLjS0iw0YWI1")
